...
for j in a c d e f g h i k l; do
	for conformer in 0 1 2 3 4 5; do
		diff=$(echo "$(awk -v line=$((conformer+1)) 'NR==line {print $1}' "$dir/xtb-goat/$j/Relative-ENERGIES-kcal-per-mol") - $(awk -v line=$((conformer+1)) 'NR==line {print $1}' "$dir/MP2/$j/Relative-ENERGIES-kcal-per-mol")" | bc)
